...
class Model:
    ...
    class SuspiciousRG:
        ...
        def lambdaFunction(self,row_reviewer1,row_reviewer2):
            ...
            if(len(commonProducts)):
                for (pid1,time1,rating1) in zip(row_reviewer1['asin'],row_reviewer1['ordinal'],row_reviewer1['overall']):
                        for (pid2,time2,rating2) in zip(row_reviewer2['asin'],row_reviewer2['ordinal'],row_reviewer2['overall']):
                            if pid1==pid2:
                                val=self.RSG(time1,rating1,time2,rating2)
                                if val == 1:
                                    return 1 , len(commonProducts)
                return 0,0
            else:
                return -1,-1
        ...
        def Graph(self,datafile,filter_data_frame):
            ...
            for index_reviewer1, row_reviewer1 in rowiter:
                ...
                for index_reviewer2, row_reviewer2 in nextrowiter:
                        reviwer_pair=[]
                        val,commonProducts=self.lambdaFunction(row_reviewer1,row_reviewer2)
                        if val == -1:
                            continue
                        elif val == 1:
                            #Graph.add_edge(row_reviewer1['reviewerID'],row_reviewer2['reviewerID'],weight=val)
                            reviwer_pair.append(row_reviewer1['reviewerID'])
                            reviwer_pair.append(row_reviewer2['reviewerID'])
                            reviwer_pair.append(1)
                            reviwer_pair.append(commonProducts)
            ...
            return graph_df,raw_data_df,reviewers_df_copy
    ...
    class Features(object):
        ...
        def __init__(self,raw_data_df,grouped_df,cliques):
            ...
            self.final_df=self.compute_features()
        ...
        def compute_features(self):
            ...
            self.getPredictions(self.raw_data_df,self.Candidate_Groups)
